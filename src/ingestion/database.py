...
class Episode(Base):
    ...
    rag_chunks = relationship("RAGChunk", back_populates="episode")
...
class RAGChunk(Base):
    """
    Optimized chunks for RAG retrieval.
    Generated by aggregating diarization chunks into semantically coherent
    pieces suitable for embedding and vector search.
    """
    ...
    # Optional speaker metadata (may include multiple)
    speakers = Column(Text, nullable=True)  # JSON list of speaker labels
    ...
    # Relationships
    episode = relationship("Episode", back_populates="rag_chunks")

    def __repr__(self):
        return f"<RAGChunk {self.chunk_index} episode_id={self.episode_id}>"
